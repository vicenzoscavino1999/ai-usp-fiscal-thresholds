...
3. For an independent live reconstruction, run scripts `01`-`15` with the
   ...
   reviewer to complete UNU-WIDER's short download form, place the three files
...
4. Compare every rebuilt file to M1. Live providers may revise data, so a new
   ...
   archived frozen parquet; to verify provenance, compare the live rebuild and
...
The public release payload contains:
...
With those files, a reviewer runs `make reproduce-public` directly. The command
